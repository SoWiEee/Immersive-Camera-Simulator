...
<template>
  <div class="app-root">
    ...
    <div v-if="store.webGPUSupported === false" class="unsupported">
      ...
      <h2>WebGPU 不支援</h2>
      <p>請使用 Chrome 113+ 以啟用 WebGPU。Firefox 需手動開啟 <code>dom.webgpu.enabled</code> flag。</p>
    </div>
    ...
    <template v-else>
      <header class="app-header">
        ...
        <span v-if="store.appState === 'ready'" class="app-header__status ready">
          模型就緒
        </span>
      </header>
...
.unsupported__icon { font-size: 3rem; }
.unsupported h2 { color: var(--text); }
.unsupported code {
...
.app-header__logo { font-weight: 600; letter-spacing: 0.05em; }
.app-header__status { font-size: 12px; }
.app-header__status.loading { color: var(--accent); }
.app-header__status.ready { color: #6bcf7f; }
